/* This file is a modified version of examples/glpsol.c
   from the GLPK distribution v5.0 which can be found
   here: https://ftp.gnu.org/gnu/glpk/glpk-5.0.tar.gz

   The original file is a standalone program which
   accepts various command-line parameters, e.g.
     -m programfile -o outputfile
   reads the GLPK program in and calculates the solution.

   I have modified it trivially
   by converting the main(int argc, char **argv)
   into glpsol(int argc, char **argv)
   So as glpsol() to be callable from any C (and XS)
   program.

   The second modification is to add a
       --verbosity L or -V L
   option to set verbosity level. By setting L to 0 the
   program is muted completely (by calling glp_term_out(GLP_OFF)).
   L > 1 is equivalent to L=1

   The object file compiled will be linked with
   lib/Algorithm/GLPK/GLPsol.xs
   The latter provides an XS function glpsol(ARRAY_REF, VERBOSITY)
   ARRAY_REF is a Perl Array ref containing all the items
   you would otherwise place in the command line when
   calling glpsol executable.

   Many thanks to the original author

     Andrew Makhorin mao cat gnu dit org

   Long live Unix, GNU and the Free Software Foundation.

   bliako cat cpan dit org, 2026
*/
...
/* BEGIN AHP : 
  glp_term_out(GLP_ON) enables xprintf to print messages to STDOUT
  glp_term_out(GLP_OFF) disables printing to STDOUT via xprintf
  END AHP
*/
...
/* BEGIN AHP */
	/* add a verbosity level allowing to mute output */
	int verbosity;
/* END AHP */
};
...
#endif
/* BEGIN AHP */
      /* Document --verbosity L */
      xprintf("   --verbosity L     set verbosity level (L) to an integer >= 0. Default is 0, meaning mute\n");
/* END AHP */
      xprintf("\n");
...
/* BEGIN AHP */
         else if (p("-V") || p("--verbosity"))
         {  k++;
            if (k == argc || argv[k][0] == '\0' || argv[k][0] == '-')
            {  xprintf("No verbosity level (integer>=0) specified\n");
               return 1;
            }
            if (str2int(argv[k], &csa->verbosity))
            {  xprintf("Invalid verbosity level '%s' -- it must be an integer >= 0\n", argv[k]);
               return 1;
            }
            if (csa->verbosity<0)
            {  xprintf("Invalid verbosity level '%s'/%d -- it must be an integer >= 0\n", argv[k], csa->verbosity);
               return 1;
            }
         }
/* END AHP */
...
#ifndef __WOE__
int glpsol(int argc, char *argv[])
#else
int __cdecl glpsol(int argc, char *argv[])
#endif
...
/* BEGIN AHP */
      /* set default verbosity to 0 = mute */
      csa->verbosity = 0;
/* END AHP */
      ...
      /* BEGIN AHP */
      if( csa->verbosity > 0 ) glp_term_out(GLP_ON); else glp_term_out(GLP_OFF);
      /* END AHP */
